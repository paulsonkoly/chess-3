...
func TestSearchStability(t *testing.T) {
	...
	for tix, tt := range tests {
		t.Run(fmt.Sprintf("stability test %d", tix), func(t *testing.T) {
			...
			score1, move1 := s.Go(b, search.WithCounters(&counters), search.WithSoftTime(10), search.WithOutput(nil))
			...
			score2, move2 := s.Go(b, search.WithCounters(&counters), search.WithSoftTime(10), search.WithOutput(nil))
			...
			score, move := s.Go(b, search.WithCounters(&counters), search.WithNodes(nodes1), search.WithOutput(nil))
			...
			score, move = s.Go(b, search.WithCounters(&counters), search.WithNodes(nodes2), search.WithOutput(nil))
			nodes = counters.Nodes
...
func TestGoNodes0(t *testing.T) {
	...
	counters := search.Counters{}
	_, move := s.Go(b, search.WithNodes(0), search.WithCounters(&counters), search.WithOutput(nil))
