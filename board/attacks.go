...
func (b *Board) Attackers(squares BitBoard, occ BitBoard, color Color) BitBoard {
	...
	for sqrs, sqBB := squares, Empty; sqrs != 0; sqrs ^= sqBB {
		sqBB = sqrs & -sqrs
		sq := sqBB.LowestSet()
...
func (b *Board) Block(squares BitBoard, color Color) BitBoard {
	...
	for square, eachSquare := Empty, squares; eachSquare != 0; eachSquare ^= square {
...
func (b *Board) IsCheckmate() bool {
	...
	for defender := Empty; defenders != 0; defenders ^= defender {
...
func (b *Board) IsStalemate() bool {
	...
	for kMove := Empty; kMoves != 0; kMoves ^= kMove {
		kMove = kMoves & -kMoves
...
func (b *Board) IsAttacked(by Color, occ, target BitBoard) bool {
	...
	for tSqr := Empty; target != 0; target ^= tSqr {
		tSqr = target & -target
		sq := tSqr.LowestSet()
...
func (b *Board) CanEnPassant(to Square) bool {
	...
	for able := Empty; ables != 0; ables ^= able {
		able = ables & -ables
