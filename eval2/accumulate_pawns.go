...
func (e *Eval[T]) addDoubledPawns(pawns BitBoard, color Color, c *CoeffSet[T]) {
	dblCnt := T(e.doubledPawns(pawns, color).Count())
	e.sp[color][MG] += c.DoubledPawns[MG] * dblCnt
	e.sp[color][EG] += c.DoubledPawns[EG] * dblCnt
}

func (e *Eval[T]) addIsolatedPawns(pawns BitBoard, color Color, c *CoeffSet[T]) {
	isoCnt := T(e.isolatedPawns(pawns, color).Count())
	e.sp[color][MG] += c.IsolatedPawns[MG] * isoCnt
	e.sp[color][EG] += c.IsolatedPawns[EG] * isoCnt
}
...
func (e *Eval[T]) addStormShelter(c *CoeffSet[T]) {
	for color := range Colors {
		...
		// central
		{
			...
			if storm := frontLine & fileBB; storm != 0 {
				...
				e.kingAttacks[color] += c.KingStorm[0][dist]
			}

			if shelter := backMost & fileBB; shelter != 0 {
				dist := Abs(kRank - shelter.LowestSet().Rank())
				e.kingAttacks[color] -= c.KingShelter[0][dist]
			} else {
				e.kingAttacks[color] += c.KingOpenFile[0]
			}
		}
		...
		// front
		{
			...
			if storm := frontLine & fileBB; storm != 0 {
				dist := Abs(kRank - storm.LowestSet().Rank())
				e.kingAttacks[color] += c.KingStorm[1][dist]
			}

			if shelter := backMost & fileBB; shelter != 0 {
				dist := Abs(kRank - shelter.LowestSet().Rank())
				e.kingAttacks[color] -= c.KingShelter[1][dist]
			} else {
				e.kingAttacks[color] += c.KingOpenFile[1]
			}
		}
		...
		// side
		{
			...
			if storm := frontLine & fileBB; storm != 0 {
				dist := Abs(kRank - storm.LowestSet().Rank())
				e.kingAttacks[color] += c.KingStorm[2][dist]
			}

			if shelter := backMost & fileBB; shelter != 0 {
				dist := Abs(kRank - shelter.LowestSet().Rank())
				e.kingAttacks[color] -= c.KingShelter[2][dist]
			} else {
				e.kingAttacks[color] += c.KingOpenFile[2]
			}
		}
	}
}
